...
def getm():
    ...
    function getpass(){
        var _0x2268f9 = Date['\x70\x61\x72\x73\x65'](new Date()) + (16798545 + -72936737 + 156138192), _0x57feae = hex_md5(_0x2268f9+'');
        const _0x5d83a3 = {};
        _0x5d83a3['\x6d'] = _0x57feae + '\u4e28' + _0x2268f9 / (-1 * 3483 + -9059 + 13542);
        return  _0x5d83a3['\x6d']
    }
                         """)
    return ctx.call("getpass")


def geturl(page):
    url = f'https://match.yuanrenxue.com/api/match/1?page={page}&m={getm()}'
    json_date = requests.get(url, headers=header).json()
    return json_date


def get_num_json(data: json):
    cha = 0
    li_data = data['data']
    for item in li_data:
        cha += item['value']
